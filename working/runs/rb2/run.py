"""GPU job 9 -- re-blocking, retested at MATCHED OPTIMIZER UPDATES.
...
def run(tag, K, seeds=(0, 1, 2, 3), **kw):
    t0 = time.time()
...
# Matched OPTIMIZER UPDATES, not matched epochs. rb1's K=2 arm trained 3x the data for the
# same 14 epochs, i.e. 3x the updates, and 22 epochs was already measured to overfit
# (0.5651 vs 0.5768). Those arms measured exposure, not re-blocking. Here every arm performs
# ~14 passes' worth of updates over the base block count.
run('K0_ep14', 0, epochs=14, edrop=0.45)
run('K2_ep5',  2, epochs=5,  edrop=0.45)
run('K6_ep2',  6, epochs=2,  edrop=0.45)
# and one arm that spends MORE updates, to separate "re-blocking helps" from "more updates help"
run('K6_ep6',  6, epochs=6,  edrop=0.45)
json.dump(RES, open(f'{ART}/metrics.json', 'w'), indent=2)
