"""GPU job 10 -- two things the incumbent has never been tested against.
...
RESIDUE_MODEL = 'igbert'      # strongest single encoder, used for part B
...
def run(tag, names, seeds=8, iface=False):
    t0 = time.time()
...
run('A_ctrl_3enc', B3)
if 'cssp' in EMB:
    run('A_plus_cssp', B3 + ['cssp'])
if 'igbertu' in EMB:
    run('A_plus_igbertu', B3 + ['igbertu'])
if RES_H is not None:
    run('B_interface', B3, iface=True)
json.dump(RES, open(f'{ART}/metrics.json', 'w'), indent=2)
